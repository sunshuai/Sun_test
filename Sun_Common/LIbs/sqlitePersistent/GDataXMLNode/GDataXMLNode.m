...
-(GDataXMLNode*)oneNodeForXPath:(NSString *)xpath error:(NSError **)error
{
	NSArray *arr = [self nodesForXPath:xpath namespaces:nil error:error];
	if(arr && [arr count] > 0)
...
- (GDataXMLElement*)oneElementForName:(NSString *)name
{
	NSArray* arr = [self elementsForName:name];
	if(arr && [arr count] > 0)
		return [arr objectAtIndex:0];
	return nil;
}
...
-(GDataXMLElement*)oneNodeForXPath:(NSString *)xpath error:(NSError **)error
{
	NSArray *arr = [self nodesForXPath:xpath namespaces:nil error:error];
	if(arr && [arr count] > 0)
		return [arr objectAtIndex:0];
